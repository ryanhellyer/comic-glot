 ...
 *
 * @uses $redis_cache Object Cache Class
 * @see Redis_DB::add()
 *
 ...
 */
function redis_cache_add( $key, $data, $group = '', $expire = 0 ) {
	global $redis_cache;

	return $redis_cache->add( $key, $data, $group, (int) $expire );
}
 ...
 */
function redis_cache_decr( $key, $offset = 1, $group = '' ) {
	global $redis_cache;

	return $redis_cache->decr( $key, $offset, $group );
}
 ...
 */
function redis_cache_delete($key, $group = '') {
	global $redis_cache;
 ...
 *		contents on success
 */
function redis_cache_get( $key, $group = '', $force = false, &$found = null ) {
	global $redis_cache;

	return $redis_cache->get( $key, $group, $force, $found );
}
 ...
 * @return false|int False on failure, the item's new value on success.
 */
function redis_cache_incr( $key, $offset = 1, $group = '' ) {
	global $redis_cache;

	return $redis_cache->incr( $key, $offset, $group );
}

/**
 * Sets up Object Cache Global and assigns it.
 *
 * @global Redis_DB $redis_cache WordPress Object Cache
 */
function redis_cache_init() {
	$GLOBALS['redis_cache'] = new Redis_DB();
}
 ...
 */
function redis_cache_replace( $key, $data, $group = '', $expire = 0 ) {
	global $redis_cache;

	return $redis_cache->replace( $key, $data, $group, (int) $expire );
}
 ...
 * @return bool False on failure, true on success
 */
function redis_cache_set( $key, $data, $group = '', $expire = 0 ) {
	global $redis_cache;

	return $redis_cache->set( $key, $data, $group, (int) $expire );
}
 ...
 */
function redis_cache_switch_to_blog( $blog_id ) {
	global $redis_cache;

	return $redis_cache->switch_to_blog( $blog_id );
}
 ...
 */
function redis_cache_add_global_groups( $groups ) {
	global $redis_cache;

	return $redis_cache->add_global_groups( $groups );
}
 ...
 * @param string|array $groups A group or an array of groups to add
 */
function redis_cache_add_non_persistent_groups( $groups ) {
	global $redis_cache;

	$redis_cache->add_non_persistent_groups( $groups );
}
 ...
 *
 * This function is deprecated. Use redis_cache_switch_to_blog() instead of this
 * function when preparing the cache for a blog switch. For clearing the cache
 * during unit tests, consider using redis_cache_init(). redis_cache_init() is not
 * recommended outside of unit tests as the performance penality for using it is
 * high.
 *
 * @deprecated 3.5.0
 */
function redis_cache_reset() {
	_deprecated_function( __FUNCTION__, '3.5' );

	global $redis_cache;

	return $redis_cache->reset();
}
